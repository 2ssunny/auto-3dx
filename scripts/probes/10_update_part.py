...
if type(parameter).__name__ != "Length":
    raise SystemExit(
        "The test parameter is not a Length. "
        f"Actual type: {type(parameter).__name__}"
    )

if "Value" not in writable_properties:
    raise SystemExit("Length.Value is not exposed as a writable property.")

try:
    original_value = parameter.Value
except com_error as exc:
    print("Original value read failed")
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    raise SystemExit(1)
...
try:
    ...
    print("Parameter value mutation failed")
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    raise SystemExit(1)
...
try:
    ...
    print("Modified value read failed")
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    raise SystemExit(1)
...
update_method = getattr(type(part), UPDATE_METHOD_NAME, None)

if not callable(update_method):
    raise SystemExit(
        f"{type(part).__name__}.{UPDATE_METHOD_NAME} "
        "is not available in the generated wrapper."
    )

try:
    value_before_update = parameter.Value
except com_error as exc:
    print("Value read before update failed")
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    raise SystemExit(1)
...
print("Value:", value_before_update)
...
try:
    ...
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    print("Document save must not be attempted.")
    raise SystemExit(1)

try:
    value_after_update = parameter.Value
except com_error as exc:
    print("Value read after update failed")
    print("HRESULT:", hex(exc.hresult & 0xFFFFFFFF))
    print("Details:", exc.excepinfo)
    raise SystemExit(1)
...
print("Update return value:", update_result)
print("Parameter:", parameter.Name)
print("Value after update:", value_after_update)
print("Document save: NOT CALLED")